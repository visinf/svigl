...
function [ mu_new, sigma_new ] = update_mu_sigma(im1, im2, mu, sigma, zsamples, energy_opt)
...
[A, b] = flow.compute_linearized_gradient_mu_sigma(im1, im2, mu, sigma, zsamples, energy_opt);
...
mu_new = reshape(full(theta_new(1:2*n*m)), n,m,2);
sigma_new = reshape(full(theta_new(2*n*m+1:end)), n,m,2);
