...
function [ A, b ] = compute_linearized_gradient_mu_sigma( noisy, mu, sigma, zsamples, energy_opt)
...
[Ac, Ax, bc, bx] = pgdenoising.compute_linearized_gradient_energy( noisy, cleansamples, energy_opt);

A_mu_mu = sparse(n*m, n*m);
A_mu_sigma = sparse(n*m, n*m);
A_sigma_mu = sparse(n*m, n*m);
A_sigma_sigma = spdiags(2./(sigma(:).^2),0,n*m,n*m); % Due to linearization of entropy gradient (Eq. 11c)
